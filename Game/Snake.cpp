...
void Snake::update(){
    ...
    age += deltaTime;
    health -= deltaTime/game_snake_health_decay;
    ...
    if(bIsAI){
        ...
        // distances to walls in FOV sections
...
void Snake::possess(DNA* dna){
    bIsAI = true;
    snake_dna = dna;
    int stage_l[game_snakenn_nb_layers] = {3*game_arena_snake_FOV_DIVISION, 12, 12 , 2};
...
void Snake::addScore(GLfloat s){
    if(!bIsAlive)return;
    score += s;
    health += game_snake_health_bonus;
    health = glm::clamp(health,0.f,game_snake_health_max);
    body.emplace_back();
    GLfloat a = angle;
    if(body.size()<2){
