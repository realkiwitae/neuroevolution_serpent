...
void Shader::CompileProgram()
{
	...
	for (size_t i = 0; i < MAX_SPOT_LIGHTS + MAX_POINT_LIGHTS; i++)
	{
		...
		snprintf(locBuff, sizeof(locBuff), "omniShadowMaps[%d].shadowMap", (int)i);
		uniformOmniShadowMap[i].uniformShadowMap = glGetUniformLocation(shaderID, locBuff);

		snprintf(locBuff, sizeof(locBuff), "omniShadowMaps[%d].farPlane", (int)i);
	...
	for (size_t i = 0; i < 6; i++)
	{
		...
		snprintf(locBuff, sizeof(locBuff), "lightMatrices[%d]", (int)i);
